...
import { dhivegaPrefixAndSuffix, ctrlKStream_userMessage, ctrlKStream_systemMessage, defaultQuickEditFimTags, rewriteCode_systemMessage, rewriteCode_userMessage, searchReplaceGivenDescription_systemMessage, searchReplaceGivenDescription_userMessage, tripleTick, } from '../common/prompt/prompts.js';
import { IDhivegaCommandBarService } from './dhivegaCommandBarService.js';
import { IKeybindingService } from '../../../../platform/keybinding/common/keybinding.js';
import { DHIVEGA_ACCEPT_DIFF_ACTION_ID, DHIVEGA_REJECT_DIFF_ACTION_ID } from './actionIDs.js';
...
import { IDhivegaSettingsService } from '../common/dhivegaSettingsService.js';
import { FeatureName } from '../common/dhivegaSettingsTypes.js';
import { IDhivegaModelService } from '../common/dhivegaModelService.js';
import { deepClone } from '../../../../base/common/objects.js';
import { acceptBg, acceptBorder, buttonFontSize, buttonTextColor, rejectBg, rejectBorder } from '../common/helpers/colors.js';
import { DiffArea, Diff, CtrlKZone, DhivegaFileSnapshot, DiffAreaSnapshotEntry, diffAreaSnapshotKeys, DiffZone, TrackingZone, ComputedDiff } from '../common/editCodeServiceTypes.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		@IDhivegaSettingsService private readonly _settingsService: IDhivegaSettingsService,
		// @IFileService private readonly _fileService: IFileService,
		@IDhivegaModelService private readonly _dhivegaModelService: IDhivegaModelService,
		@IConvertToLLMMessageService private readonly _convertToLLMMessageService: IConvertToLLMMessageService,
		// @ts-ignore - Used in the file but TypeScript doesn't detect it
		@IDhivegaCommandBarService private readonly _dhivegaCommandBarService: IDhivegaCommandBarService,
	) {
		super();
		...
		const initializeModel = async (model: ITextModel) => {
			...
			await this._dhivegaModelService.initializeModel(model.uri)
	...
	// 		message: `Dhivega Error: ${e.message}`,
	// 		actions: {
	// 			secondary: [{
	// 				id: 'dhivega.onerror.opensettings',
	// 				enabled: true,
	// 				label: `Open Dhivega's settings`,
	// 				tooltip: '',
	// 				class: undefined,
	// 				run: () => { this._commandService.executeCommand(DHIVEGA_OPEN_SETTINGS_ACTION_ID) }
	// 			}]
	// 		},
	// 		source: details ? `(Hold ${isMacintosh ? 'Option' : 'Alt'} to hover) - ${details}\n\nIf this persists, feel free to [report](https://github.com/yourusername/dhivega/issues/new) it.` : undefined
	// 	})
	// }
	...
	private _addDiffAreaStylesToURI = (uri: URI) => {
		const { model } = this._dhivegaModelService.getModel(uri)
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			if (diffArea.type === 'DiffZone') {
				...
				if (diffArea._streamState.isStreaming) {
					// sweepLine ... sweepLine
					const fn1 = this._addLineDecoration(model, diffArea._streamState.line, diffArea._streamState.line, 'dhivega-sweepIdxBG')
					// sweepLine+1 ... endLine
					const fn2 = diffArea._streamState.line + 1 <= diffArea.endLine ?
						this._addLineDecoration(model, diffArea._streamState.line + 1, diffArea.endLine, 'dhivega-sweepBG')
						: null
				...
				const fn = this._addLineDecoration(model, diffArea.startLine, diffArea.endLine, 'dhivega-highlightBG')
	...
	private _addDiffStylesToURI = (uri: URI, diff: Diff) => {
		...
		if (type !== 'deletion') {
			const fn = this._addLineDecoration(model, diff.startLine, diff.endLine, 'dhivega-greenBG', {
		...
		if (type !== 'insertion') {
			const consistentZoneId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					const domNode = document.createElement('div');
					domNode.className = 'dhivega-redBG'
		...
		if (diffZone.type === 'DiffZone' && !diffZone._streamState.isStreaming) {
			...
			const consistentWidgetId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					else { throw new Error('Dhivega Error 1') }
	...
	private _getCurrentDhivegaFileSnapshot = (uri: URI): DhivegaFileSnapshot => {
		const { model } = this._dhivegaModelService.getModel(uri)
	...
	private _restoreDhivegaFileSnapshot = async (uri: URI, snapshot: DhivegaFileSnapshot) => {
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		const beforeSnapshot: DhivegaFileSnapshot = this._getCurrentDhivegaFileSnapshot(uri)
		let afterSnapshot: DhivegaFileSnapshot | null = null
		...
		const elt: IUndoRedoElement = {
			...
			undo: async () => { opts?.onWillUndo?.(); await this._restoreDhivegaFileSnapshot(uri, beforeSnapshot) },
			redo: async () => { if (afterSnapshot) await this._restoreDhivegaFileSnapshot(uri, afterSnapshot) }
		}
		this._undoRedoService.pushElement(elt)

		const onFinishEdit = async () => {
			afterSnapshot = this._getCurrentDhivegaFileSnapshot(uri)
			await this._dhivegaModelService.saveModel(uri)
		}
		return { onFinishEdit }
	}


	public getDhivegaFileSnapshot(uri: URI) {
		return this._getCurrentDhivegaFileSnapshot(uri)
	}


	public restoreDhivegaFileSnapshot(uri: URI, snapshot: DhivegaFileSnapshot): void {
		this._restoreDhivegaFileSnapshot(uri, snapshot)
	}
	...
	private _writeStreamedDiffZoneLLMText(uri: URI, originalCode: string, llmTextSoFar: string, deltaText: string, latestMutable: StreamLocationMutable) {
		...
		if (!lastDiff) {
			...
			if (lastDiff.type === 'insertion' || lastDiff.type === 'edit')
				...
				throw new Error(`Dhivega: diff.type not recognized on: ${lastDiff}`)
		}
	...
	public async callBeforeApplyOrEdit(givenURI: URI | 'current') {
		...
		if (!uri) return
		await this._dhivegaModelService.initializeModel(uri)
		await this._dhivegaModelService.saveModel(uri) // save the URI
	}
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<void>] | undefined {
		...
		if (from === 'ClickApply') {
			...
			throw new Error(`Dhivega: diff.type not recognized on: ${from}`)
		}
		...
		if (from === 'ClickApply') {
			...
			const { prefix, suffix } = dhivegaPrefixAndSuffix({ fullFileStr: originalFileCode, startLine, endLine })
		...
		const extractText = (fullText: string, recentlyAddedTextLen: number) => {
			...
			throw new Error('Dhivega Error 1')
		}
			...
			while (shouldSendAnotherMessage) {
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onFinalMessage: async (params) => {
						...
						const blocks = extractSearchReplaceBlocks(fullText)
						if (blocks.length === 0) {
							this._notificationService.info(`Dhivega: We ran Fast Apply, but the LLM didn't output any changes.`)
						}
						this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
	...
	public async acceptDiff({ diffid }: { diffid: number }) {
		...
		if (diff.type === 'deletion') {
			...
			throw new Error(`Dhivega error: ${diff}.type not recognized`)
		}
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		const getAcceptRejectText = () => {
			const acceptKeybinding = this._keybindingService.lookupKeybinding(DHIVEGA_ACCEPT_DIFF_ACTION_ID);
			const rejectKeybinding = this._keybindingService.lookupKeybinding(DHIVEGA_REJECT_DIFF_ACTION_ID);
			...
			const commandBarStateAtUri = this._dhivegaCommandBarService.stateOfURI[uri.fsPath];
		...
		this._register(editor.onDidScrollChange((e: any) => { updateTop() }))
		this._register(editor.onDidChangeModelContent((e: any) => { updateTop() }))
		this._register(editor.onDidLayoutChange((e: any) => { updateTop(); updateLeft() }))


		// Listen for state changes in the command bar service
		this._register(this._dhivegaCommandBarService.onDidChangeState(e => {
