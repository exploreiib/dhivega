...
import { mountDhivegaCommandBar } from './react/out/dhivega-editor-widgets-tsx/index.js'
...
import { DHIVEGA_ACCEPT_DIFF_ACTION_ID, DHIVEGA_REJECT_DIFF_ACTION_ID, DHIVEGA_GOTO_NEXT_DIFF_ACTION_ID, DHIVEGA_GOTO_PREV_DIFF_ACTION_ID, DHIVEGA_GOTO_NEXT_URI_ACTION_ID, DHIVEGA_GOTO_PREV_URI_ACTION_ID, DHIVEGA_ACCEPT_FILE_ACTION_ID, DHIVEGA_REJECT_FILE_ACTION_ID, DHIVEGA_ACCEPT_ALL_DIFFS_ACTION_ID, DHIVEGA_REJECT_ALL_DIFFS_ACTION_ID } from './actionIDs.js';
...
export interface IDhivegaCommandBarService {
...
export const IDhivegaCommandBarService = createDecorator<IDhivegaCommandBarService>('DhivegaCommandBarService');
...
export class DhivegaCommandBarService extends Disposable implements IDhivegaCommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'dhivega.DhivegaCommandBarService'
	...
	// Get editCodeService through service locator to avoid circular dependency
	private get _editCodeService(): IEditCodeService {
		return this._instantiationService.invokeFunction(accessor => accessor.get(IEditCodeService));
	}
	...
	constructor(
		...
		// @IEditCodeService private readonly _editCodeService: IEditCodeService, // Removed to break circular dependency
	...
	async goToURIIdx(idx: number | null): Promise<void> {
		...
		const { model } = await this._dhivegaModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(IDhivegaCommandBarService, DhivegaCommandBarService, InstantiationType.Delayed); // delayed is needed here :(


export type DhivegaCommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountDhivegaCommandBar(root, accessor, { uri, editor } satisfies DhivegaCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies DhivegaCommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: DHIVEGA_ACCEPT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('dhivegaAcceptDiffAction', 'Dhivega: Accept Diff'),
			keybinding: {
				primary: KeyMod.CtrlCmd | KeyMod.Alt | KeyMod.Shift | KeyCode.Enter,
				mac: { primary: KeyMod.WinCtrl | KeyMod.Alt | KeyCode.Enter },
				weight: KeybindingWeight.DhivegaExtension,
			}
		});
	}
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const commandBarService = accessor.get(IDhivegaCommandBarService);
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: DHIVEGA_REJECT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('dhivegaRejectDiffAction', 'Dhivega: Reject Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: DHIVEGA_GOTO_NEXT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('dhivegaGoToNextDiffAction', 'Dhivega: Go to Next Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: DHIVEGA_GOTO_PREV_DIFF_ACTION_ID,
			f1: true,
			title: localize2('dhievgaGoToPrevDiffAction', 'Dhivega: Go to Previous Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: DHIVEGA_GOTO_NEXT_URI_ACTION_ID,
			f1: true,
			title: localize2('dhivegaGoToNextUriAction', 'Dhievga: Go to Next File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: DHIVEGA_GOTO_PREV_URI_ACTION_ID,
			f1: true,
			title: localize2('dhivegaGoToPrevUriAction', 'Dhivega: Go to Previous File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: DHIVEGA_ACCEPT_FILE_ACTION_ID,
			f1: true,
			title: localize2('dhivegaAcceptFileAction', 'Dhivega: Accept All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: DHIVEGA_REJECT_FILE_ACTION_ID,
			f1: true,
			title: localize2('dhivegaRejectFileAction', 'Dhivega: Reject All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: DHIVEGA_ACCEPT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('dhivegaAcceptAllDiffsAction', 'Dhivega: Accept All Diffs in All Files'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: DHIVEGA_REJECT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('dhivegaRejectAllDiffsAction', 'Dhivega: Reject All Diffs in All Files'),
			keybinding: {
