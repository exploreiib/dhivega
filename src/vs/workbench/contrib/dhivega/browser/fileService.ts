...
class FilePromptActionService extends Action2 {
	private static readonly DHIVEGA_COPY_FILE_PROMPT_ID = 'dhivega.copyfileprompt'

	constructor() {
		super({
			id: FilePromptActionService.DHIVEGA_COPY_FILE_PROMPT_ID,
			title: localize2('dhivegaCopyPrompt', 'Dhivega: Copy Prompt'),
			menu: [{
		...
		try {
			...
			if (stat.isFile) {
				m = await messageOfSelection({
					type: 'File',
					uri,
					language: (await dhivegaModelService.getModelSafe(uri)).model?.getLanguageId() || '',
