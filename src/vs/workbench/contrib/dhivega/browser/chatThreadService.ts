...
import { FeatureName, ModelSelection, ModelSelectionOptions } from '../common/dhivegaSettingsTypes.js';
...
import { DhivegaFileSnapshot } from '../common/editCodeServiceTypes.js';
...
export const IChatThreadService = createDecorator<IChatThreadService>('dhivegaChatThreadService');
class ChatThreadService extends Disposable implements IChatThreadService {
	...
	private _setState(state: Partial<ThreadsState>, doNotRefreshMountInfo?: boolean) {
		...
		if (streamState?.isRunning === undefined && !streamState?.error) {
			...
			// if awaiting user but stream state doesn't indicate it (happens if restart Dhivega)
			if (lastMessage && lastMessage.role === 'tool' && lastMessage.type === 'tool_request')
				this._setStreamState(threadId, { isRunning: 'awaiting_user', })

			// if running now but stream state doesn't indicate it (happens if restart Dhivega), cancel that last tool
	...
	private _getCheckpointInfo = (checkpointMessage: ChatMessage & { role: 'checkpoint' }, fsPath: string, opts: { includeUserModifiedChanges: boolean }) => {
		const dhivegaFileSnapshot = checkpointMessage.dhivegaFileSnapshotOfURI ? checkpointMessage.dhivegaFileSnapshotOfURI[fsPath] ?? null : null
		if (!opts.includeUserModifiedChanges) { return { dhivegaFileSnapshot, } }

		const userModifiedDhivegaFileSnapshot = fsPath in checkpointMessage.userModifications.dhivegaFileSnapshotOfURI ? checkpointMessage.userModifications.dhivegaFileSnapshotOfURI[fsPath] ?? null : null
		return { dhivegaFileSnapshot: userModifiedDhivegaFileSnapshot ?? dhivegaFileSnapshot, }
	}
	...
	private _computeNewCheckpointInfo({ threadId }: { threadId: string }) {
		...
		const dhivegaFileSnapshotOfURI: { [fsPath: string]: DhivegaFileSnapshot | undefined } = {}
		...
		for (const fsPath in lastIdxOfURI ?? {}) {
			const { model } = this._dhivegaModelService.getModelFromFsPath(fsPath)
			if (!model) continue
			...
			if (!res) continue
			const { dhivegaFileSnapshot: oldDhivegaFileSnapshot } = res
			...
			const dhivegaFileSnapshot = this._editCodeService.getDhivegaFileSnapshot(URI.file(fsPath))
			if (oldDhivegaFileSnapshot === dhivegaFileSnapshot) continue
			dhivegaFileSnapshotOfURI[fsPath] = dhivegaFileSnapshot
		}
		...
		return { dhivegaFileSnapshotOfURI }
	}


	private _addUserCheckpoint({ threadId }: { threadId: string }) {
		const { dhivegaFileSnapshotOfURI } = this._computeNewCheckpointInfo({ threadId }) ?? {}
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'user_edit',
			dhivegaFileSnapshotOfURI: dhivegaFileSnapshotOfURI ?? {},
			userModifications: { dhivegaFileSnapshotOfURI: {}, },
		})
	}
	...
	private _addToolEditCheckpoint({ threadId, uri, }: { threadId: string, uri: URI }) {
		...
		const diffAreasSnapshot = this._editCodeService.getDhivegaFileSnapshot(uri)
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'tool_edit',
			dhivegaFileSnapshotOfURI: { [uri.fsPath]: diffAreasSnapshot },
			userModifications: { dhivegaFileSnapshotOfURI: {} },
		})
	}
	...
	private _getCheckpointsBetween({ threadId, loIdx, hiIdx }: { threadId: string, loIdx: number, hiIdx: number }) {
		...
		for (let i = loIdx; i <= hiIdx; i += 1) {
			...
			for (const fsPath in message.dhivegaFileSnapshotOfURI) { // do not include userModified.beforeStrOfURI here, jumping should not include those changes
	...
	private _addUserModificationsToCurrCheckpoint({ threadId }: { threadId: string }) {
		...
		this._editMessageInThread(threadId, checkpointIdx, {
			...checkpoint,
			userModifications: { dhivegaFileSnapshotOfURI: dhivegaFileSnapshotOfURI ?? {}, },
		})
	}
		...
		if (toIdx < fromIdx) {
			...
			for (const fsPath in lastIdxOfURI) {
				...
				for (const k of idxes()) {
					...
					if (!res) continue
					const { dhivegaFileSnapshot } = res
					if (!dhivegaFileSnapshot) continue
					this._editCodeService.restoreDhivegaFileSnapshot(URI.file(fsPath), dhivegaFileSnapshot)
					break
				}
			}
		}
	...
	generateCodespanLink: IChatThreadService['generateCodespanLink'] = async ({ codespanStr: _codespanStr, threadId }) => {
		...
		if (codespanType === 'function-or-class') {
			...
			for (const uri of prevUris) {
				...
				const modelRef = await this._dhivegaModelService.getModelSafe(uri)
