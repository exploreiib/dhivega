...
import { displayInfoOfProviderName, ProviderName, providerNames, localProviderNames, featureNames, FeatureName, isFeatureNameDisabled } from '../../../../common/dhivegaSettingsTypes.js';
import { ChatMarkdownRender } from '../markdown/ChatMarkdownRender.js';
import { OllamaSetupInstructions, OneClickSwitchButton, SettingsForProvider, ModelDump } from '../dhivega-settings-tsx/Settings.js';
...
export const DhivegaOnboarding = () => {
	...
	return (
		<div className={`@@dhivega-scope ${isDark ? 'dark' : ''}`}>
			<div
				className={`
					bg-dhivega-bg-3 fixed top-0 right-0 bottom-0 left-0 width-full z-[99999]
...
const AddProvidersPage = ({ pageIndex, setPageIndex }: { pageIndex: number, setPageIndex: (index: number) => void }) => {
	...
	return (<div className="flex flex-col md:flex-row w-full h-[80vh] gap-6 max-w-[900px] mx-auto relative">
		{/* Left Column */}
		<div className="md:w-1/4 w-full flex flex-col gap-6 p-6 border-none border-dhivega-border-2 h-full overflow-y-auto">
			{/* Tab Selector */}
		...
		<div className="flex-1 flex flex-col items-center justify-start p-6 h-full overflow-y-auto">
			...
			<div className="w-full max-w-xl mt-4 mb-10">
				...
				<div className="text-sm opacity-80 text-dhivega-fg-3 my-4 w-full">{descriptionOfTab[currentTab]}</div>
			</div>
			...
			{providerNamesOfTab[currentTab].map((providerName) => (
				<div key={providerName} className="w-full max-w-xl mb-10">
					<div className="text-xl mb-2">
						...
						{providerName === 'gemini' && (
							<span
								data-tooltip-id="dhivega-tooltip-provider-info"
			...
			{(currentTab === 'Local' || currentTab === 'Cloud/Other') && (
				<div className="w-full max-w-xl mt-8 bg-dhivega-bg-2/50 rounded-lg p-6 border border-dhivega-border-4">
					<div className="flex items-center gap-2 mb-4">
						<div className="text-xl font-medium">Models</div>
					</div>

					{currentTab === 'Local' && (
						<div className="text-sm opacity-80 text-dhivega-fg-3 my-4 w-full">Local models should be detected automatically. You can add custom models below.</div>
					)}
...
// 		title:
// 			div
// 				"How would you like to use Dhivega?"
// 		content:
...
const PreviousButton = ({ onClick, ...props }: { onClick: () => void } & React.ButtonHTMLAttributes<HTMLButtonElement>) => {
	return (
		<button
			onClick={onClick}
			className="px-6 py-2 rounded text-dhivega-fg-3 opacity-80 hover:brightness-115 duration-600 transition-all"
			{...props}
		>
			Back
		</button>
...
const OllamaDownloadOrRemoveModelButton = ({ modelName, isModelInstalled, sizeGb }: { modelName: string, isModelInstalled: boolean, sizeGb: number | false | 'not-known' }) => {
	...
	return <a
		...
		className="flex items-center justify-center text-dhivega-fg-2 hover:text-dhivega-fg-1"
	>
...
const DhivegaOnboardingContent = () => {


	const accessor = useAccessor()
	const dhivegaSettingsService = accessor.get('IDhivegaSettingsService')
	const dhivegaMetricsService = accessor.get('IMetricsService')
		...
		<div className="flex items-center gap-2">
			...
			<PrimaryActionButton
				onClick={() => {
					...
					dhivegaMetricsService.capture('Completed Onboarding', { selectedProviderName, wantToUseOption })
				}}
				ringSize={dhivegaSettingsState.globalSettings.isOnboardingComplete ? 'screen' : undefined}
			>Enter the Dhivega</PrimaryActionButton>
		</div>
	</div>
	...
	const detailedDescOfWantToUseOption: { [wantToUseOption in WantToUseOption]: string } = {
		...
		private: "Private-hosted so your data never leaves your computer or network. [Email us](mailto:founders@dhivega.com) for help setting up at your company.",
		...
		0: <OnboardingPageShell
			content={
				<div className='flex flex-col items-center gap-8'>
					<div className="text-5xl font-light text-center">Welcome to Dhivega</div>

					{/* Slice of Dhivega image */}
			...
			content={
				<div>
					...
					<div className="mt-8 text-center flex flex-col items-center gap-4 w-full max-w-md mx-auto">
						<h4 className="text-dhivega-fg-3 mb-4">Transfer your settings from an existing editor?</h4>
