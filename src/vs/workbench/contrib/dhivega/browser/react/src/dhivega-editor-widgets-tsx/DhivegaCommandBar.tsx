...
import { DhivegaCommandBarProps } from '../../../dhivegaCommandBarService.js';
import { Check, EllipsisVertical, Menu, MoveDown, MoveLeft, MoveRight, MoveUp, X } from 'lucide-react';
import {
	DHIVEGA_GOTO_NEXT_DIFF_ACTION_ID,
	DHIVEGA_GOTO_PREV_DIFF_ACTION_ID,
	DHIVEGA_GOTO_NEXT_URI_ACTION_ID,
	DHIVEGA_GOTO_PREV_URI_ACTION_ID,
	DHIVEGA_ACCEPT_FILE_ACTION_ID,
	DHIVEGA_REJECT_FILE_ACTION_ID,
	DHIVEGA_ACCEPT_ALL_DIFFS_ACTION_ID,
	DHIVEGA_REJECT_ALL_DIFFS_ACTION_ID
} from '../../../actionIDs.js';

export const DhivegaCommandBarMain = ({ uri, editor }: DhivegaCommandBarProps) => {
	const isDark = useIsDark()

	return <div
		className={`@@dhivega-scope ${isDark ? 'dark' : ''}`}
	>
		<DhivegaCommandBar uri={uri} editor={editor} />
	</div>
}
...
export const DhivegaCommandBar = ({ uri, editor }: DhivegaCommandBarProps) => {
	...
	const _upKeybinding = keybindingService.lookupKeybinding(DHIVEGA_GOTO_PREV_DIFF_ACTION_ID);
	const _downKeybinding = keybindingService.lookupKeybinding(DHIVEGA_GOTO_NEXT_DIFF_ACTION_ID);
	const _leftKeybinding = keybindingService.lookupKeybinding(DHIVEGA_GOTO_PREV_URI_ACTION_ID);
	const _rightKeybinding = keybindingService.lookupKeybinding(DHIVEGA_GOTO_NEXT_URI_ACTION_ID);
	const _acceptFileKeybinding = keybindingService.lookupKeybinding(DHIVEGA_ACCEPT_FILE_ACTION_ID);
	const _rejectFileKeybinding = keybindingService.lookupKeybinding(DHIVEGA_REJECT_FILE_ACTION_ID);
	const _acceptAllKeybinding = keybindingService.lookupKeybinding(DHIVEGA_ACCEPT_ALL_DIFFS_ACTION_ID);
	const _rejectAllKeybinding = keybindingService.lookupKeybinding(DHIVEGA_REJECT_ALL_DIFFS_ACTION_ID);
	...
	if (currFileIdx === null) {
		return (
			<div className="pointer-events-auto">
				<div className="flex bg-dhivega-bg-2 shadow-md border border-dhivega-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:border-r [&>*]:border-dhivega-border-2 [&>*:last-child]:border-r-0">
		...
		<div className="pointer-events-auto">
			...
			{showAcceptRejectAllButtons && showAcceptRejectAll && (
				<div className="flex justify-end mb-1">
					<div className="inline-flex bg-dhivega-bg-2 rounded shadow-md border border-dhivega-border-2 overflow-hidden">
						<div className="flex items-center [&>*]:border-r [&>*]:border-dhivega-border-2 [&>*:last-child]:border-r-0">
			...
			<div className="flex items-center bg-dhivega-bg-2 rounded shadow-md border border-dhivega-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:px-3 [&>*]:border-r [&>*]:border-dhivega-border-2 [&>*:last-child]:border-r-0">
