...
import { mountDhivegaSelectionHelper } from './react/out/dhivega-editor-widgets-tsx/index.js';
...
export type DhivegaSelectionHelperProps = {
...
export class SelectionHelperContribution extends Disposable implements IEditorContribution, IOverlayWidget {
	public static readonly ID = 'editor.contrib.dhivegaSelectionHelper';
	// react
	...
	constructor(
		...
		@IDhivegaSettingsService private readonly _dhivegaSettingsService: IDhivegaSettingsService
	) {
		super();
		...
		this._instantiationService.invokeFunction(accessor => {
			...
			const res = mountDhivegaSelectionHelper(content, accessor);
			if (!res) return;
	...
	private _showHelperForSelection(selection: Selection): void {
		...
		// rerender
		const enabled = this._dhivegaSettingsService.state.globalSettings.showInlineSuggestions
		...
		if (enabled) {
			this._rerender({ rerenderKey: this._rerenderKey } satisfies DhivegaSelectionHelperProps)
