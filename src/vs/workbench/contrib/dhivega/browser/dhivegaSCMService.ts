...
import { IDhivegaSCMService } from '../common/dhivegaSCMTypes.js'
import { IMainProcessService } from '../../../../platform/ipc/common/mainProcessService.js'
import { IDhivegaSettingsService } from '../common/dhivegaSettingsService.js'
import { IConvertToLLMMessageService } from './convertToLLMMessageService.js'
import { ILLMMessageService } from '../common/sendLLMMessageService.js'
import { ModelSelection, OverridesOfModel, ModelSelectionOptions } from '../common/dhivegaSettingsTypes.js'
...
export const IGenerateCommitMessageService = createDecorator<IGenerateCommitMessageService>('dhivegaGenerateCommitMessageService');

const loadingContextKey = 'dhivegaSCMGenerateCommitMessageLoading'
...
class GenerateCommitMessageService extends Disposable implements IGenerateCommitMessageService {
	...
	private dhivegaSCM: IDhivegaSCMService
	private loadingContextKey: IContextKey<boolean>

	constructor(
		@ISCMService private readonly scmService: ISCMService,
		@IMainProcessService mainProcessService: IMainProcessService,
		@IDhivegaSettingsService private readonly dhivegaSettingsService: IDhivegaSettingsService,
		...
		this.dhivegaSCM = ProxyChannel.toService<IDhivegaSCMService>(mainProcessService.getChannel('dhivega-channel-scm'))
	}
		...
		this.execute.trigger(async () => {
			...
			try {
				...
				const [stat, sampledDiffs, branch, log] = await Promise.all([
					this.dhivegaSCM.gitStat(path),
					this.dhivegaSCM.gitSampledDiffs(path),
					this.dhivegaSCM.gitBranch(path),
					this.dhivegaSCM.gitLog(path)
				])

				if (!this.isCurrentRequest(requestId)) { throw new CancellationError() }

				const modelSelection = this.dhivegaSettingsService.state.modelSelectionOfFeature['SCM'] ?? null
				const modelSelectionOptions = modelSelection ? this.dhivegaSettingsService.state.optionsOfModelSelection['SCM'][modelSelection?.providerName]?.[modelSelection.modelName] : undefined
				const overridesOfModel = this.dhivegaSettingsService.state.overridesOfModel
	...
	private sendLLMMessage(messages: LLMChatMessage[], separateSystemMessage: string, modelOptions: ModelOptions): Promise<string> {
		return new Promise((resolve, reject) => {
			...
			this.llmRequestId = this.llmMessageService.sendLLMMessage({
				...
				logging: { loggingName: 'DhivegaSCM - Commit Message' },
			})
		})
	}
	...
	private onError(error: any) {
		if (!isCancellationError(error)) {
			console.error(error)
			this.notificationService.error(localize2('dhivegaFailedToGenerateCommitMessage', 'Failed to generate commit message.').value)
		}
	}
}
...
class GenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'dhivega.generateCommitMessageAction',
			title: localize2('dhivegaCommitMessagePrompt', 'Dhivega: Generate Commit Message'),
			icon: ThemeIcon.fromId('sparkle'),
			tooltip: localize2('dhivegaCommitMessagePromptTooltip', 'Dhivega: Generate Commit Message'),
			f1: true,
			menu: [{
...
class LoadingGenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'dhivega.loadingGenerateCommitMessageAction',
			title: localize2('dhivegaCommitMessagePromptCancel', 'Dhivega: Cancel Commit Message Generation'),
			icon: ThemeIcon.fromId('stop-circle'),
			tooltip: localize2('dhivegaCommitMessagePromptCancelTooltip', 'Dhivega: Cancel Commit Message Generation'),
