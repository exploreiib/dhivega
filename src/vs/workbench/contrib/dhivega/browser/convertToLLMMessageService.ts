...
import { ChatMode, FeatureName, ModelSelection, ProviderName } from '../common/dhivegaSettingsTypes.js';
...
const prepareOpenAIOrAnthropicMessages = ({
	...
	if (aiInstructions) sysMsgParts.push(`GUIDELINES (from the user's .dhivegarules file):\n${aiInstructions}`)
...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	constructor(
		...
		@IDhivegaModelService private readonly dhivegaModelService: IDhivegaModelService,
		@IMCPService private readonly mcpService: IMCPService,
	) {
		super()
	}

	// Read .dhivegarules files from workspace folders
	private _getDhivegaRulesFileContents(): string {
		try {
			const workspaceFolders = this.workspaceContextService.getWorkspace().folders;
			let dhivegaRules = '';
			for (const folder of workspaceFolders) {
				const uri = URI.joinPath(folder.uri, '.dhivegarules')
				const { model } = this.dhivegaModelService.getModel(uri)
				if (!model) continue
				dhivegaRules += model.getValue(EndOfLinePreference.LF) + '\n\n';
			}
			return dhivegaRules.trim();
		}
		catch (e) {
			return ''
		}
	...
	// Get combined AI instructions from settings and .dhivegarules files
	private _getCombinedAIInstructions(): string {
		const globalAIInstructions = this.dhivegaSettingsService.state.globalSettings.aiInstructions;
		const dhivegaRulesFileContent = this._getDhivegaRulesFileContents();

		const ans: string[] = []
		if (globalAIInstructions) ans.push(globalAIInstructions)
		if (dhivegaRulesFileContent) ans.push(dhivegaRulesFileContent)
		return ans.join('\n\n')
	}
	...
	prepareLLMSimpleMessages: IConvertToLLMMessageService['prepareLLMSimpleMessages'] = ({ simpleMessages, systemMessage, modelSelection, featureName }) => {
		...
		const { overridesOfModel } = this.dhivegaSettingsService.state
		...
		const modelSelectionOptions = this.dhivegaSettingsService.state.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]
	...
	prepareLLMChatMessages: IConvertToLLMMessageService['prepareLLMChatMessages'] = async ({ chatMessages, chatMode, modelSelection }) => {
		...
		const { disableSystemMessage } = this.dhivegaSettingsService.state.globalSettings;
		const fullSystemMessage = await this._generateChatMessagesSystemMessage(chatMode, specialToolFormat)
		const systemMessage = disableSystemMessage ? '' : fullSystemMessage;

		const modelSelectionOptions = this.dhivegaSettingsService.state.optionsOfModelSelection['Chat'][modelSelection.providerName]?.[modelSelection.modelName]
