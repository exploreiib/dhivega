...
import { DHIVEGA_VIEW_CONTAINER_ID, DHIVEGA_VIEW_ID } from './sidebarPane.js';
import { IMetricsService } from '../common/metricsService.js';
import { ICommandService } from '../../../../platform/commands/common/commands.js';
import { DHIVEGA_TOGGLE_SETTINGS_ACTION_ID } from './dhivegaSettingsPane.js';
...
const DHIVEGA_OPEN_SIDEBAR_ACTION_ID = 'dhivega.sidebar.open'
registerAction2(class extends Action2 {
	constructor() {
		super({ id: DHIVEGA_OPEN_SIDEBAR_ACTION_ID, title: localize2('dhivegaOpenSidebar', 'Dhivega: Open Sidebar'), f1: true });
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		const viewsService = accessor.get(IViewsService)
		const chatThreadsService = accessor.get(IChatThreadService)
		viewsService.openViewContainer(DHIVEGA_VIEW_CONTAINER_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: DHIVEGA_CTRL_L_ACTION_ID,
			f1: true,
			title: localize2('dhivegaCmdL', 'Dhivega: Add Selection to Chat'),
			keybinding: {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		// open panel
		const wasAlreadyOpen = viewsService.isViewContainerVisible(DHIVEGA_VIEW_CONTAINER_ID)
		if (!wasAlreadyOpen) {
			await commandService.executeCommand(DHIVEGA_OPEN_SIDEBAR_ACTION_ID)
		}
...
const DHIVEGA_CMD_SHIFT_L_ACTION_ID = 'dhivega.cmdShiftL'
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: DHIVEGA_CMD_SHIFT_L_ACTION_ID,
			title: 'New Chat',
			keybinding: {
			...
			icon: { id: 'add' },
			menu: [{ id: MenuId.ViewTitle, group: 'navigation', when: ContextKeyExpr.equals('view', DHIVEGA_VIEW_ID), }],
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		commandService.executeCommand(DHIVEGA_CMD_SHIFT_L_ACTION_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'dhivega.settingsAction',
			title: `Dhivega's Settings`,
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		const commandService = accessor.get(ICommandService)
		commandService.executeCommand(DHIVEGA_TOGGLE_SETTINGS_ACTION_ID)
	}
})
