...
export class DhivegaMainUpdateService extends Disposable implements IDhivegaUpdateService {
	...
	async check(explicit: boolean): Promise<DhivegaCheckUpdateRespose> {
		...
		if (this._updateService.state.type === StateType.Ready) {
			// Update is ready
			return { message: 'Restart Dhivega to update!', action: 'restart' } as const
		}
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<DhivegaCheckUpdateRespose> {
		try {
			const response = await fetch('https://api.github.com/repos/yourusername/dhivega/releases/latest');
			...
			if (explicit) {
				if (response.ok) {
					if (!isUpToDate) {
						message = 'A new version of Dhivega is available! Please reinstall (auto-updates are disabled on this OS) - it only takes a second!'
						action = 'reinstall'
					}
					else {
						message = 'Dhivega is up-to-date!'
					}
				}
				else {
