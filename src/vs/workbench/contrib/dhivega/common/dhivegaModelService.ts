...
type DhivegaModelType = {
	model: ITextModel | null;
	editorModel: IResolvedTextEditorModel | null;
};

export interface IDhivegaModelService {
	readonly _serviceBrand: undefined;
	initializeModel(uri: URI): Promise<void>;
	getModel(uri: URI): DhivegaModelType;
	getModelFromFsPath(fsPath: string): DhivegaModelType;
	getModelSafe(uri: URI): Promise<DhivegaModelType>;
	saveModel(uri: URI): Promise<void>;

}

export const IDhivegaModelService = createDecorator<IDhivegaModelService>('dhivegaModelService');

class DhivegaModelService extends Disposable implements IDhivegaModelService {
	_serviceBrand: undefined;
	static readonly ID = 'dhivegaModelService';
	...
	getModelSafe = async (uri: URI): Promise<DhivegaModelType> => {
...
registerSingleton(IDhivegaModelService, DhivegaModelService, InstantiationType.Eager);
