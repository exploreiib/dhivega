...
import { IDhivegaSettingsService } from './dhivegaSettingsService.js';
import { ILLMMessageService } from './sendLLMMessageService.js';
import { Emitter, Event } from '../../../../base/common/event.js';
import { Disposable, IDisposable } from '../../../../base/common/lifecycle.js';
import { RefreshableProviderName, refreshableProviderNames, SettingsOfProvider } from './dhivegaSettingsTypes.js';
...
export class RefreshModelService extends Disposable implements IRefreshModelService {
	...
	constructor(
		...
		const initializeAutoPollingAndOnChange = () => {
			...
			if (!dhivegaSettingsService.state.globalSettings.autoRefreshModels) return
			...
			for (const providerName of refreshableProviderNames) {
				...
				let relevantVals = () => refreshBasedOn[providerName].map(settingName => dhivegaSettingsService.state.settingsOfProvider[providerName][settingName])
				let prevVals = relevantVals() // each iteration of a for loop has its own context and vars, so this is ok
				disposables.add(
					dhivegaSettingsService.onDidChangeState(() => { // we might want to debounce this
		...
		dhivegaSettingsService.waitForInitState.then(() => {
			initializeAutoPollingAndOnChange()
			this._register(
				dhivegaSettingsService.onDidChangeState((type: any) => { if (typeof type === 'object' && type[1] === 'autoRefreshModels') initializeAutoPollingAndOnChange() })
			)
		})
	...
	startRefreshingModels: IRefreshModelService['startRefreshingModels'] = (providerName, options) => {
		...
		const autoPoll = () => {
			if (this.dhivegaSettingsService.state.globalSettings.autoRefreshModels) {
				// resume auto-polling
		...
		listFn({
			...
			onSuccess: ({ models }) => {
				...
				this.dhivegaSettingsService.setAutodetectedModels(
					providerName,
					models.map(model => {
				...
				if (options.enableProviderOnSuccess) this.dhivegaSettingsService.setSettingOfProvider(providerName, '_didFillInProviderSettings', true)
