...
import { DHIVEGA_SETTINGS_STORAGE_KEY } from './storageKeys.js';
import { defaultSettingsOfProvider, FeatureName, ProviderName, ModelSelectionOfFeature, SettingsOfProvider, SettingName, providerNames, ModelSelection, modelSelectionsEqual, featureNames, DhivegaStatefulModelInfo, GlobalSettings, GlobalSettingName, defaultGlobalSettings, ModelSelectionOptions, OptionsOfModelSelection, ChatMode, OverridesOfModel, defaultOverridesOfModel, MCPUserStateOfName as MCPUserStateOfName, MCPUserState } from './dhivegaSettingsTypes.js';
...
export type DhivegaSettingsState = {
...
// type RealDhivegaSettings = Exclude<keyof DhivegaSettingsState, '_modelOptions'>
// type EventProp<T extends RealDhivegaSettings = RealDhivegaSettings> = T extends 'globalSettings' ? [T, keyof DhivegaSettingsState[T]] : T | 'all'


export interface IDhivegaSettingsService {
	readonly _serviceBrand: undefined;
	readonly state: DhivegaSettingsState; // in order to play nicely with react, you should immutably change state
	...
	dangerousSetState(newState: DhivegaSettingsState): Promise<void>;
...
const _modelsWithSwappedInNewModels = (options: { existingModels: DhivegaStatefulModelInfo[], models: string[], type: 'autodetected' | 'default' }) => {
	const { existingModels, models, type } = options

	const existingModelsMap: Record<string, DhivegaStatefulModelInfo> = {}
...
const _stateWithMergedDefaultModels = (state: DhivegaSettingsState): DhivegaSettingsState => {
...
const _validatedModelState = (state: Omit<DhivegaSettingsState, '_modelOptions'>): DhivegaSettingsState => {
	...
	} satisfies DhivegaSettingsState
...
export const IDhivegaSettingsService = createDecorator<IDhivegaSettingsService>('DhivegaSettingsService');
class DhivegaSettingsService extends Disposable implements IDhivegaSettingsService {
	_serviceBrand: undefined;

	private readonly _onDidChangeState = new Emitter<void>();
	readonly onDidChangeState: Event<void> = this._onDidChangeState.event; // this is primarily for use in react, so react can listen + update on state changes

	state: DhivegaSettingsState;
	...
	dangerousSetState = async (newState: DhivegaSettingsState) => {
	...
	async readAndInitializeState() {
		let readS: DhivegaSettingsState
		try {
	...
	private async _readState(): Promise<DhivegaSettingsState> {
		const encryptedState = this._storageService.get(DHIVEGA_SETTINGS_STORAGE_KEY, StorageScope.APPLICATION)
	...
	private async _storeState() {
		...
		this._storageService.store(DHIVEGA_SETTINGS_STORAGE_KEY, encryptedState, StorageScope.APPLICATION, StorageTarget.USER);
	}
	...
	setGlobalSetting: SetGlobalSettingFn = async (settingName, newVal) => {
		const newState: DhivegaSettingsState = {
			...this.state,
			globalSettings: {
	...
	toggleModelHidden(providerName: ProviderName, modelName: string) {
		...
		const newModels: DhivegaStatefulModelInfo[] = [
...
registerSingleton(IDhivegaSettingsService, DhivegaSettingsService, InstantiationType.Eager);
